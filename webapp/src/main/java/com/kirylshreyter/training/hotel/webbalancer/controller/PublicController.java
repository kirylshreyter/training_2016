...
public class PublicController {
	...
	private ResponseEntity<List<Object>> getAllAvailableRoom(@RequestBody String data,
		...
		try {
			...
			/*
			 * for(int i=0; i<dataJsonArray.length; i++) { JSONObject dataObj =
			 * dataJsonArray.get(i); String id = dataObj.getString("id");
			 * //Similarly you can extract for other fields. } Object object =
			 * getInstance.getInstanceFromObjectString(response.getFirstHeader(
			 * "entity").getValue()); for (int i = 0; i < myObject.size(); i++){
			 * Object obj = myObject.get(i); Method getMethod; getMethod =
			 * object.getClass().getMethod("getId"); Object id =
